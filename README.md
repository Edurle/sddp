...
| 层级 | 技术 |
|------|------|
| 后端 | Python 3.12, FastAPI, SQLAlchemy (async), SQLite (aiosqlite), Motor/MongoDB, Pydantic, JWT |
| 前端 | Vue 3, TypeScript, Pinia, Vue Router, Axios, Vite |
| 测试 | pytest + pytest-asyncio (后端), Playwright (前端 E2E) |
| 数据库 | SQLite（关系数据）+ MongoDB（规格文档/模板） |
...
- Python 3.12 + Conda
- Node.js 18+
- MongoDB（规格文档存储，默认 `localhost:27017`）
...
│   │   ├── mongo_models/    # Motor/MongoDB 文档模型
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `DATABASE_URL` | `sqlite+aiosqlite:///./test.db` | SQLite 连接字符串 |
