...
class TestDefaultTemplateAgentPrompt:
    ...
    async def test_default_template_has_agent_prompt_on_all_fields(self):
        ...
        expected_prompts = {
            ...
            ("entity_definition", "fields"): "列出实体的所有字段。每个字段需包含 name（字段名，英文小写下划线）、type（数据类型，如 string/integer/boolean/datetime/json）、description（字段含义的中文描述）、constraints（约束数组，如 ['required', 'unique', 'max:255'])",
